...
private:
    ...
    _<ATimer> mDiaryTimer;
    _<ATimer> mPersonalityTimer;
    // OpenAITools mTools;

    bool mWakeup = false;

    /**
     * @brief Set by mDiaryTimer (fires every 12h); consumed at the top of the main loop to dump the diary
     * regardless of accumulated token count. Consumed (rather than dumping directly from the timer) so it only
     * ever runs between notification-processing turns, never concurrently with one.
     */
    bool mDiaryDumpRequested = false;

    /**
     * @brief Set by mPersonalityTimer (fires rarely, see personalityConsolidationInterval); consumed at the
     * top of the main loop, same pattern as mDiaryDumpRequested, so it never runs concurrently with a
     * notification-processing turn.
     */
    bool mPersonalityConsolidationRequested = false;
